Attribute VB_Name = "modLearnerWebsiteSync_v2"
Option Explicit
...
Public Sub SetupLearnerWebsiteSync()
    ...
    ws.Range("A1").Value = "Math & Sciences Hub - Learner API Sync"
    ws.Range("A3").Value = "Private Data Folder"
    ws.Range(CFG_FOLDER_CELL).Value = IIf(Len(ThisWorkbook.Path) > 0, ThisWorkbook.Path, "")
    ws.Range("A4").Value = "Private JSON File Name"
    ws.Range(CFG_FILE_CELL).Value = "learners_private.json"
    ...
    ws.Range("A8").Value = "Security note: export learner data to a backend data folder, not to the public website folder."
    ws.Range("A9").Value = "The backend/API should read this file server-side and return only one verified learner record at a time."
    ws.Range("A11").Value = "Recommended backend path example: C:\MSH_Backend\data\learners_private.json"

    ws.Columns("A:B").AutoFit

    MsgBox "Learner backend sync has been set up on the '" & CONFIG_SHEET & "' sheet." & vbCrLf & _
           "Set the private data folder, then run ExportLearnersJson.", vbInformation, "MSH Learner API Sync"
End Sub
...
Public Sub ChooseLearnerJsonFolder()
    ...
    With fd
        .Title = "Select Backend Data Folder for learners_private.json"
        .AllowMultiSelect = False
        If .Show <> -1 Then Exit Sub
        ws.Range(CFG_FOLDER_CELL).Value = .SelectedItems(1)
    End With

    MsgBox "Backend data folder saved to " & CONFIG_SHEET & "!" & CFG_FOLDER_CELL, vbInformation, "MSH Learner API Sync"
End Sub
...
Public Sub ExportLearnersJson()
    ...
    fileName = GetConfigValue(CFG_FILE_CELL, "learners_private.json")
    activeOnly = IsYesValue(GetConfigValue(CFG_ACTIVE_ONLY_CELL, "Yes"))

    If Len(folderPath) = 0 Then
        MsgBox "Please choose or enter the backend data folder on " & CONFIG_SHEET & "!" & CFG_FOLDER_CELL & ".", vbExclamation, "MSH Learner API Sync"
        GoTo CleanExit
    End If
    ...
    MsgBox exportedCount & " learner record(s) exported to:" & vbCrLf & fullPath & vbCrLf & vbCrLf & _
           skippedCount & " record(s) were skipped (inactive or missing ID Number).", vbInformation, "MSH Learner API Sync"
...
CleanFail:
    Application.ScreenUpdating = True
    Application.EnableEvents = True
    MsgBox "Learner JSON export failed: " & Err.Description, vbCritical, "MSH Learner API Sync"
End Sub
...
Public Sub OpenLearnerWebsiteFolder()
    ...
    If Len(folderPath) = 0 Then
        MsgBox "No backend data folder has been set yet.", vbExclamation, "MSH Learner API Sync"
        Exit Sub
    End If
